#ifndef DCFIDUCIALCUTS_HPP
#define DCFIDUCIALCUTS_HPP
...
#include <cstring>
#include <string>
//#include "readCaloBank.hpp"

auto determineSector(float x, float y, float z) -> int;

auto dcFiducialCutXY(int dc_sector, int region, float x, float y, int partPID, bool isInbending) -> bool;

auto loadMapByIndex(hipo::bank &fromBank, const std::string &idxVarName) -> std::map<int, std::vector<int>> {
    std::map<int, std::vector<int>> map;

    if (fromBank.getRows() > 0) {
        for (int iFrom = 0; iFrom < fromBank.getRows(); iFrom++) {
            int iTo = fromBank.getInt(idxVarName.c_str(), iFrom);
	    	    //cout<<iTo<<";"<<map.size();
            if (map.find(iTo) == map.end()) map.insert({iTo, std::vector<int>()});

            map.at(iTo).emplace_back(iFrom);
...
auto DC_cuts_electron(hipo::bank &RECTraj, const int &pindex) -> bool {
    int pid      = 11;
    auto nan     = std::numeric_limits<float>::quiet_NaN();
    //RECTraj.show();
    auto trajMap = loadMapByIndex(RECTraj, "pindex");

    float x1 = nan, y1 = nan, z1 = nan, x2 = nan, y2 = nan, z2 = nan, x3 = nan, y3 = nan, z3 = nan;
    for (int i_traj: trajMap.at(pindex)) {
        if (RECTraj.getInt("detector", i_traj) == 6 and RECTraj.getInt("layer", i_traj) == 6) {
            x1 = RECTraj.getFloat("x", i_traj);
            y1 = RECTraj.getFloat("y", i_traj);
            z1 = RECTraj.getFloat("z", i_traj);
        }
        if (RECTraj.getInt("detector", i_traj) == 6 and RECTraj.getInt("layer", i_traj) == 18) {
            x2 = RECTraj.getFloat("x", i_traj);
            y2 = RECTraj.getFloat("y", i_traj);
            z2 = RECTraj.getFloat("z", i_traj);
        }
        if (RECTraj.getInt("detector", i_traj) == 6 and RECTraj.getInt("layer", i_traj) == 36) {
            x3 = RECTraj.getFloat("x", i_traj);
            y3 = RECTraj.getFloat("y", i_traj);
            z3 = RECTraj.getFloat("z", i_traj);
        }
    }

    if (x1 == nan or y1 == nan or x2 == nan or y2 == nan or x3 == nan or y3 == nan) return false;
    bool cut_DC1 = dcFiducialCutXY(determineSector(x1, y1, z1), 1, x1, y1, pid, true);
    bool cut_DC2 = dcFiducialCutXY(determineSector(x2, y2, z2), 2, x2, y2, pid, true);
    bool cut_DC3 = dcFiducialCutXY(determineSector(x3, y3, z3), 3, x3, y3, pid, true);
    return cut_DC1 and cut_DC2 and cut_DC3;
}

auto determineSector(float x, float y, float z) -> int {
    float pi = 3.14;
    double phi = 180.0 / M_PI * std::atan2(y / sqrt(x * x + y * y + z * z), x / sqrt(x * x + y * y + z * z));
...
auto dcFiducialCutXY(int dc_sector, int region, float x, float y, int partPID, bool isInbending) -> bool {
    ...
    double maxParamsIn[6][6][3][2] = {{{{-14.563, 0.60032}, {-19.6768, 0.58729}, {-22.2531, 0.544896}},
    ...
    double minParamsIn[6][6][3][2] = {{{{12.2692, -0.583057}, {17.6233, -0.605722}, {19.7018, -0.518429}},
    ...
    double maxParamsOut[6][6][3][2] = {{{{-9.86221, 0.565985}, {-16.4397, 0.569087}, {-29.7787, 0.586842}},
    ...
    double minParamsOut[6][6][3][2] = {{{{8.07831, -0.548881}, {16.4382, -0.569075}, {33.7768, -0.607402}},
    ...
    auto minParams = isInbending ? minParamsIn : minParamsOut;
    auto maxParams = isInbending ? maxParamsIn : maxParamsOut;

    double X = x;
    double Y = y;
    float pi = 3.14;

    auto toRadians = [&pi](float a) { return pi / 180.0 * a; };

    double X_new = X * std::cos(toRadians(-60.f * (static_cast<float>(dc_sector) - 1.f))) -Y * std::sin(toRadians(-60.0f * (static_cast<float>(dc_sector) - 1.f)));
    Y = X * std::sin(toRadians(-60.f * (static_cast<float>(dc_sector) - 1.f))) +Y * std::cos(toRadians(-60.0f * (static_cast<float>(dc_sector) - 1.f)));
    X = X_new;
    ...
    if (isInbending and partPID == 11) pid = 0;
