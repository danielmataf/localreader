...
//g++ -o testcratio testcratio.cpp `root-config --cflags --libs`
// ./testcratio
  ...
  for (Int_t i = 1; i <= 10; ++i)
    {
        ...
        hist_cratio.w_cQ_Sn->Fill(Q2_Sn, cos(phih_Sn));
        hist_cratio.w_cQ_De->Fill(Q2_D, cos(phih_D));
        hist_cratio.w_cv_Sn->Fill(v_Sn, cos(phih_Sn));
        hist_cratio.w_cv_De->Fill(v_D, cos(phih_D));
        hist_cratio.w_cz_Sn->Fill(z_Sn, cos(phih_Sn));
        hist_cratio.w_cz_De->Fill(z_D, cos(phih_D));
        hist_cratio.w_cpt_De->Fill(P_t_Sn, cos(phih_Sn));
        hist_cratio.w_cpt_De->Fill(P_t_D, cos(phih_D));
    }
...
for (int i_q=1; i_q<=hist_cratio.w_cQ_Sn->GetNbinsX(); i_q++) {
        double x_q_w1 = hist_cratio.w_cQ_Sn->GetXaxis()->GetBinCenter(i_q);		
        double y_q_wSn1 = hist_cratio.w_cQ_Sn->GetBinContent(i_q);				
        double x_q1 = h_QtestSn->GetXaxis()->GetBinCenter(i_q);			
        double y_qSn1 = h_QtestSn->GetBinContent(i_q) ;
	    double y_q_wDe1 = hist_cratio.w_cQ_De->GetBinContent(i_q);
	...
	//double Rq_err=  (1/y_q_Sn) + (1/y_q_De) + (1/y_q_Sn_e) + (1/y_q_De_e); 
		if(x_q_w1>1.5){ 
        	    cos_Q->SetPoint(i_q-1, x_q_w1,meancQSn1/ meancQDe1);
    	}
	//}
}

for (int i_v=1; i_v<=hist_cratio.w_cQ_Sn->GetNbinsX(); i_v++) {
        double x_v_w1 = hist_cratio.w_cQ_Sn->GetXaxis()->GetBinCenter(i_v);		
        double y_v_wSn1 = hist_cratio.w_cQ_Sn->GetBinContent(i_v);				
        double x_v1 = h_vtestSn->GetXaxis()->GetBinCenter(i_v);			
        double y_vSn1 = h_vtestSn->GetBinContent(i_v) ;
	    double y_v_wDe1 = hist_cratio.w_cQ_De->GetBinContent(i_v);
	...
	//double Rq_err=  (1/y_v_Sn) + (1/y_v_De) + (1/y_v_Sn_e) + (1/y_v_De_e); 
		//if(x_v_w<8.5 && x_v_w>2.5){ 
        	    cos_v->SetPoint(i_v-1, x_v_w1,meancvSn1/ meancvDe1);
    	//}
	//}
}

for (int i_z=1; i_z<=hist_cratio.w_cQ_Sn->GetNbinsX(); i_z++) {
        double x_z_w1 = hist_cratio.w_cQ_Sn->GetXaxis()->GetBinCenter(i_z);		
        double y_z_wSn1 = hist_cratio.w_cQ_Sn->GetBinContent(i_z);				
        double x_z1 = h_ztestSn->GetXaxis()->GetBinCenter(i_z);			
        double y_zSn1 = h_ztestSn->GetBinContent(i_z) ;
	    double y_z_wDe1 = hist_cratio.w_cQ_De->GetBinContent(i_z);
	...
	//double Rq_err=  (1/y_z_Sn) + (1/y_z_De) + (1/y_z_Sn_e) + (1/y_z_De_e); 
		//if(x_z_w<8.5 && x_z_w>2.5){ 
        	    cos_z->SetPoint(i_z-1, x_z_w1,meanczSn1/ meanczDe1);
    	//}
	//}
}

for (int i_pt=1; i_pt<=hist_cratio.w_cQ_Sn->GetNbinsX(); i_pt++) {
        double x_pt_w1 = hist_cratio.w_cQ_Sn->GetXaxis()->GetBinCenter(i_pt);		
        double y_pt_wSn1 = hist_cratio.w_cQ_Sn->GetBinContent(i_pt);				
        double x_pt1 = h_pttestSn->GetXaxis()->GetBinCenter(i_pt);			
        double y_ptSn1 = h_pttestSn->GetBinContent(i_pt) ;
	    double y_pt_wDe1 = hist_cratio.w_cQ_De->GetBinContent(i_pt);
	...
	//double Rq_err=  (1/y_pt_Sn) + (1/y_pt_De) + (1/y_pt_Sn_e) + (1/y_pt_De_e); 
		//if(x_pt_w<8.5 && x_pt_w>2.5){ 
        	    cos_pt->SetPoint(i_pt-1, x_pt_w1,meancptSn1/ meancptDe1);
    	//}
	//}
}
...
cos_Q->GetYaxis()->SetTitle("<cos(#Phi)>_{Sn} / <cos(#Phi)>_{D}");
cos_Q->Draw("AP");
...
cos_v->GetYaxis()->SetTitle("<cos(#Phi)>_{Sn} / <cos(#Phi)>_{D}");
//cos_v->Draw("AP");
...
cos_z->GetYaxis()->SetTitle("<cos(#Phi)>_{Sn} / <cos(#Phi)>_{D}");
//cos_z->Draw("AP");
cR->cd(4);
...
cos_pt->GetYaxis()->SetTitle("<cos(#Phi)>_{Sn} / <cos(#Phi)>_{D}");
cos_pt->Draw("AP");

    cR->SaveAs("testcratio.pdf");
    cR->SaveAs("testcratio.root");
