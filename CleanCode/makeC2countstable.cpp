// makeC2countstable.cpp
// Extract (xb,theta,z,pt2) bin contents from cos_<SCHEME>_<TARGET>.root
// and write a flat table: xb  theta  z  pt2  N  Nw  Nw2
//
// Build:
//   g++ makeC2countstable.cpp -o makeC2countstable $(root-config --cflags --libs)
// Run examples:
//   ./makeC2countstable --target Sn
//   ./makeC2countstable --scheme BIS --target Cu
...
static void usage(const char* p){
  std::cerr
    ...
    << "Reads  : cos_2<SCHEME>_<TARGET>.root  (expects dirs: dist, w_cos, w_cos2)\n"
    << "Writes : C2counts_<SCHEME>_<TARGET>.txt\n";
}
...
int main(int argc, char** argv){
  ...
  const std::string inRoot = "cos_2" + cfg.scheme + "_" + cfg.target + ".root";
  ...
  const std::string outTxt = "C2counts_" + cfg.scheme + "_" + cfg.target + ".txt";
