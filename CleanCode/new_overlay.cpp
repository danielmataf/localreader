...
// Compile & run:
// g++ new_overlay.cpp -o new_overlay `root-config --cflags --glibs`
// ./plot_xQ2_overlay

constexpr double M = 0.9396;
constexpr double E = 10.5;

constexpr double THETA_HI  = 20.0;
constexpr double THETA_MID = 12;
constexpr double THETA_LOW = 8;
...
int main() {
    TFile* fin = TFile::Open("~/Desktop/rootJUL8/REcutC2_test.root", "READ");
    if (!fin || fin->IsZombie()) {
    ...
    std::vector<double> xBins = {0.11, 0.16, 0.28, 0.38};  // → 5 xB regions
    ...
    // y = 0.75 line    
    double y_fixed = 0.75;  //change y value here 
    double s = 2.0 * M * E;
    ...
    // W² = 4 GeV² line fixed because low cut 
    ...
    // W² = 8 GeV² additional line to change bining
    TF1* fW2_high = new TF1("fW2_hi", "(9 - 0.9396*0.9396)*x/(1 - x)", 0.0, 0.7);
    fW2_high->SetLineColor(kOrange + 1);
    fW2_high->SetLineWidth(2);
    fW2_high->SetLineStyle(1);
    fW2_high->SetMinimum(1.0);

    //W² = 6 GeV² additiona line
    TF1* fW2_mid = new TF1("fW2_mid", "(6 - 0.9396*0.9396)*x/(1 - x)", 0.0, 0.7);
    fW2_mid->SetLineColor(kOrange + 1);
    fW2_mid->SetLineWidth(2);
    fW2_mid->SetLineStyle(1);
    fW2_mid->SetMinimum(1.0);
    ...
    leg->AddEntry(fW2_mid, "W^{2} = 6 GeV^{2}", "l");
    leg->AddEntry(fW2_high, "W^{2} = 8 GeV^{2}", "l");
    leg->Draw();

    c->SaveAs("newverlay.png");
