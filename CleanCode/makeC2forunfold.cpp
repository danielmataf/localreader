...
// How to compile:
// g++ makeC2forunfold.cpp -o makeC2forunfold $(root-config --cflags --libs)
//
// Default: phih in degrees (converted internally)
// ./makeC2forunfold
//
//# If your phih branch is in radians:
//./makeC2forunfold --phi-rad
//--------------------------------------
...
void WriteScheme(TTree* T, const Grid& G, const std::string& tag) {
  const std::string outFile = "cos_2" + tag + "_" + TARGET_TAG + ".root";
  ...
  for (Long64_t i=0; i<nEntries; ++i) {
    ...
    double c  = std::cos(2 * phi_for_cos);
    double c2 = c*c;
