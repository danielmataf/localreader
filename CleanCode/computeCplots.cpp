// computeCplots.cpp
//
// Cos(phi_h) ratio plots.
// - Reads Ccounts_<SCHEME>_<TARGET>.txt (columns: xb th z pt N Nw Nw2)
// - Produces 5 PDFs (pt=1..5) for A/LD2 with targets CxC, Cu, Sn
// - Produces 5 PDFs (pt=1..5) for C1/C2 self-ratio
//
// Compile:
//   g++ computeCplots.cpp -o computeCplots $(root-config --cflags --libs)
// Run:
//                  # scheme=DAT (default)
//   ./computeCplots --scheme BIS
//
#include <TROOT.h>
#include <TStyle.h>
#include <TCanvas.h>
...
static void drawFrameAndTitle(TCanvas& c,
  ...
  frame->GetYaxis()->SetTitle("<cos#phi_{h}>_{A}/<cos#phi_{h}>_{D}");
  frame->SetTitle("");
...
int main(int argc, char** argv){
  ...
  auto path = [&](const std::string& T){ return "Ccounts_" + cfg.scheme + "_" + T + ".txt"; };
  ...
  for (int ipt=1; ipt<=Npt; ++ipt){
    const std::string pdf = "Cratio_" + cfg.scheme + "_A_over_LD2_pt" + std::to_string(ipt) + ".pdf";
    int page=0;
  ...
  // ---------- C1/C2 PDFs ----------
  for (int ipt=1; ipt<=Npt; ++ipt){
    const std::string pdf = "Cratio_" + cfg.scheme + "_C1_over_C2_pt" + std::to_string(ipt) + ".pdf";
    int page=0;
    ...
    for (int ith=1; ith<=Nth; ++ith){
      for (int ixb=1; ixb<=Nx; ++ixb){
        ...
        drawFrameAndTitle(
          c, x1, x2, 0.6, 1.4,
          Form("<cos#phi_{h}> self-ratio  C1/C2  |  scheme=%s  |  x_{B} [%g,%g]  #theta_{e} [%g,%g] deg  p_{T}^{2} [%g,%g] GeV^{2}",
               cfg.scheme.c_str(),
               G.x[ixb-1], G.x[ixb], G.th[ith-1], G.th[ith],
               PT2_EDGES[ipt-1], PT2_EDGES[ipt])
        );
